...
function buildUi() {
  ...
  panel.innerHTML = `
    ...
    <p class="diagnostic-note">Profilinsamlingen accepterar nu jämn rotation och visar exakt varför en profil klassas som osäker.</p>`;
...
async function loadAnalysis() {
  ...
  try {
    ...
    await importTimed('../vision/auto-laser-profile-collector-v56.js', 'rörelseanpassad profilinsamling');
    state.analysisLoaded = true;
    if (button) button.textContent = 'Laseranalys laddad';
    if (status) status.textContent = 'Laseranalys laddad';
    if (detail) detail.textContent = 'Jämn rotation accepteras; T-rotation är fortfarande avstängd';
    log('analysis ready med rörelseanpassad profilbedömning');
  } catch (error) {
